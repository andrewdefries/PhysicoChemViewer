...
library(ggplot2)
#####################################
files <- list.files(recursive=FALSE, pattern = ".csv")
hw <- read.csv(files[4])
#####################################
...
shinyServer(function(input, output) {
  ...
  output$main_plot <- renderPlot({
    ...
    # Separate by activity, if requested
    if (input$Bioactivity) {
      aes_mapping <- aes_string(x = input$x_var, y = input$y_var,
                                colour = "Bioactivity", shape = "Bioactivity")
  ...
  output$mod_linear_text <- renderPrint({
    ...
    # Use activity as a predictor variable
    if (input$Bioactivity) {
      formula <- paste(formula, " * Bioactivity", sep = "")
    }
